"""
plots.py — Visualizaciones para Civilization-ABM v2.
...
def _save(fig, path: Path | str, dpi: int = DPI):
    path = Path(path)
...
def plot_baseline_panel(model_df: pd.DataFrame, output_path: str = None) -> plt.Figure:
    ...
    axes[0].plot(steps, model_df["Gini"], color="#1565C0", linewidth=1.5)
    ...
    axes[1].plot(steps, model_df["MeanWealth"], color="#388E3C", linewidth=1.5)
    axes[1].set_xlabel("Step"); axes[1].set_ylabel("Mean wealth")
    axes[1].set_title("Mean Agent Wealth")

    axes[2].plot(steps, model_df["AliveAgents"], color="#6A1B9A", linewidth=1.5)
...
def plot_strategy_evolution(model_df: pd.DataFrame, output_path: str = None) -> plt.Figure:
    ...
    for strategy, col in [("Cooperative", "CoopShare"),
                           ("Competitive", "CompShare"),
                           ("Neutral",     "NeutralShare")]:
        if col in model_df.columns:
            color = PALETTE.get(strategy.lower(), "gray")
            axes[0].plot(steps, model_df[col], label=strategy, color=color, linewidth=1.5)
    ...
    if "StrategyEntropy" in model_df.columns:
        axes[1].plot(steps, model_df["StrategyEntropy"], color="#795548", linewidth=1.5)
...
def plot_institutional_collapse(model_df: pd.DataFrame, output_path: str = None) -> plt.Figure:
    ...
    if "Stability" in model_df.columns:
        axes[0].plot(steps, model_df["Stability"], color="#1565C0", linewidth=1.5)
    ...
    if "Regime" in model_df.columns:
        regimes = model_df["Regime"].values
...
def plot_results_heatmap(
    ...
    ax.set_title(f"{metric.replace('_', ' ').title()} by {row_var} × {col_var}")
...
def plot_comparison_panel(
    ...
    fig.suptitle("Cross-condition Comparison — Civilization-ABM v2", fontsize=12)
    fig.tight_layout()
    if output_path:
        _save(fig, output_path)
    return fig
